        ...
        <Card className="p-0 border-slate-800/50 bg-slate-900/20 flex flex-col h-[400px]">
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-2">
                {history.length === 0 ? (
                    ...
                    history.map((entry) => (
                        <div key={entry.id} className="p-3 rounded-lg border border-slate-800 bg-slate-950/40 hover:bg-slate-900/40 transition-all group">
                            <div className="flex justify-between items-start mb-2">
                                <div className="flex items-center gap-2">
                                    <span className="text-[10px] font-mono text-slate-400">#{entry.id.toString().padStart(3, '0')}</span>
                            ...
                            <div className="grid grid-cols-3 gap-2 mt-2 pt-2 border-t border-slate-800/50">
                                <div>
                                    <div className="text-[8px] text-slate-400 uppercase font-mono tracking-tighter">Cost</div>
                                    <div className="text-[10px] text-indigo-400 font-mono tabular-nums">${entry.cost.toFixed(6)}</div>
                                </div>
                                <div>
                                    <div className="text-[8px] text-slate-400 uppercase font-mono tracking-tighter">Tokens</div>
                                    <div className="text-[10px] text-slate-400 font-mono tabular-nums">{entry.tokens}</div>
                                </div>
                                <div>
                                    <div className="text-[8px] text-slate-400 uppercase font-mono tracking-tighter">Policy</div>
                                    <div className="text-[10px] text-slate-400 font-mono truncate uppercase">{entry.strategy}</div>
                                </div>
                            </div>
                        </div>
                    ))
